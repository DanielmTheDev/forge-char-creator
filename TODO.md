...
- [ ] **Recharge no-op on squires-mark + rending-pounce** — both declare Recharge 5–6 but their activities have no `consumption`, so the use is never spent → recharge meaningless (usable every turn). Fix = add `consumption.targets:[{type:"itemUses",target:"",value:"1"}]` + usesSpent gate asserts (see #5). (found 2026-06-05)
