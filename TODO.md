...
- [x] **BUG-3. Char-creator picker showed no descriptions** — DONE (2026-08-11). Hit while building the Scalewoven Overseer: compendium search listed spells by name only. Root cause `app.js` `#performSearch` — `getIndex({fields:["name","img"]})`, and descriptions can't ride the index at all (dnd5e packs don't declare `system.description.value` as an index field), so the full doc must load. Fix: `type` badge from the index (free) + lazy `#loadDescription(uuid)` (`fromUuid` → enrichHTML → plain-text) cached per app instance, eager for the first 5 rows, debounced-on-hover/arrow-key for the rest; bin pills get the text as a tooltip; dropdown background made opaque (theme var is translucent, sheet bled through the now-taller rows). Search UUIDs normalized to the `Compendium.<pack>.Item.<id>` form. New `tests/search-descriptions.spec.js`. Item creation path untouched — created spells always HAD descriptions.
...
- [x] **Wizard windows unscrollable** — FIXED. Core V13 `.window-content` is `overflow:hidden` and `.application` is capped by `max-height`, so a tall part root was silently clipped (Create button unreachable). `PARTS[].scrollable` only saves/restores scrollTop — it adds NO overflow. Fix: `.forge-form-body` scroll wrapper + `<footer class="form-footer">` in char-creator/effect-creator templates; part root is now `flex:1 1 auto; min-height:0; overflow:hidden`. NOTE `flex-basis` MUST be `auto` — `flex:1` (basis 0%) + `min-height:0` collapses auto-height windows to zero height (broke the hub; caught by `tests/window-scroll.spec.js` collapse case). (found+fixed 2026-08-03)
